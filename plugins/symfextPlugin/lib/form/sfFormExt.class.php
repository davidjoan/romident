...
class sfFormExt extends sfFormSymfony
{
   ...
   * Update validators.
   *
   * Updates validator schema using a ValidatorBuilder object
   ...
   *
   * When making some changes and wants to 
   * rebuild the form.
   * 
   ...
   * of this form.
   *
   * @param boolean $strip_html If the html tags should be stripped from the string
   *
   * @return string             The rendered errors
   *
   * @todo Test
   */
  public function renderErrors($strip_html = false)
  {
    $errors = array();
    ...
    $ret = $this->getWidgetSchema()->getFormFormatter()->formatErrorRow($errors);
    
    return $strip_html ? strip_tags($ret) : $ret;
  }
  
  /**
   * Executes some methods before binding the tainted values and files.
   *
   * @see sfForm
   */
  public function bind(array $taintedValues = null, array $taintedFiles = null)
  {
    $taintedValues = $this->updateTaintedValues($taintedValues);
    $taintedFiles  = $this->updateTaintedFiles($taintedFiles);
  ...
  /**
   * Updates the tainted values before doing the bind.
   * 
   * @param array $taintedValues   An array of input values
   */
  public function updateTaintedValues(array $taintedValues = null)
  {
    return $taintedValues;
  }
  
  /**
   * Updates the tainted files before doing the bind.
   * 
   * @param array $taintedFiles    An array of uploaded files (in the $_FILES or $_GET format)
   */
  public function updateTaintedFiles(array $taintedFiles = null)
  {
    return $taintedFiles;
  }
  ...
  /**
   * Adds some labels to the labels array
   * 
   * @param array $labels The labels to add
   */
  ...
  /**
   * Adds some types to the types array
   * 
   * @param array $types The types to add
   */
  ...
  /**
   * Returns the a value from the request object.
   * 
   * @param string $field     The field of the form
   * @param string $form_name The form name
   * 
   * @return string The value from the field in the request
   */
   ...
   *
   * @return sfContext The current context
   */
   ...
   *
   * @return sfRouting The current routing
   */
   ...
   *
   * @return sfUser The current user
   */
   ...
   */
  protected function genUrl($uri, $absolute = false)
  {
    return $this->getContext()->getController()->genUrl($uri, $absolute);
  }
}
